...
class ResultsAggregation:
    ...
    def analyse_correlation_matrix(self, core_features, target_col='BTC-USD_signal_voladj_10d'):
        ...
        stability_df = self.permutation_stability(model=model, X=X, y=y, n_splits=5, n_repeats=5)
        stability_df["stability_class"] = pd.cut(
        ...
        role_df = self.role_consistency_table(perm_df=self.perm_df, stock_name = self.stock, features=final_features, horizons=[5, 10, 20])
        print("-"*40)
        print('role_df:')
        print(role_df)
